...
public class ColorChooser extends JPanel implements ChangeListener, KeyListener {
	...
	JSlider blue_slider;
	JLabel color_label;
	List<ColorListener> color_listeners;

	public ColorChooser(Color init_color) {
		color = init_color;
		prior_color = init_color;
		...
		color_listeners = new ArrayList<ColorListener>();
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (slider.getValueIsAdjusting()) {
			...
			ColorChosenEvent ce = new ColorChosenEvent(prior_color, color);
			
			for (ColorListener l : color_listeners) {
				l.colorChanged(ce);
			}
